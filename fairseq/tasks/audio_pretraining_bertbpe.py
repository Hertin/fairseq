...
class AudioPretrainingConfig(FairseqDataclass):
    ...
    bert_path: str = field(
        default="",
        metadata={"help": "path of bert model"},
    )


@register_task("audio_pretraining_bertbpe", dataclass=AudioPretrainingConfig)
class AudioPretrainingBertBPETask(FairseqTask):
    """"""
    ...
    def encode(self, sentence):
        bpe_sentence = "<s> " + self.bert.bpe.encode(sentence)
    ...
    def _inference_with_wer(self, generator, sample, model):
        ...
        def decode(toks):
            s = self.bert.decode(toks.int().cpu())
            return s
