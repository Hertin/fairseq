...
@register_model("wav2bart_chr", dataclass=Wav2BartChrConfig)
class Wav2BartChr(FairseqEncoderDecoderModel):
    def __init__(self, encoder, decoder):
        super().__init__(encoder, decoder)

    @classmethod
    def build_model(cls, cfg: Wav2BartChrConfig, task: FairseqTask):
        """Build a new model instance."""

        assert cfg.autoregressive, "Please set task.autoregressive=true for seq2seq asr models"

        # print('Wav2BartChr task',task, task.target_dictionary)

        encoder = cls.build_encoder(cfg)
        
        decoder = cls.build_decoder(cfg, task.target_dictionary)
        model = Wav2BartChr(encoder, decoder)
        return model

    @classmethod
    def build_encoder(cls, cfg: Wav2BartChrConfig):
    ...
    @classmethod
    def build_decoder(cls, cfg: Wav2BartChrConfig, tgt_dict):
        decoder = BartDecoder(cfg, dictionary=tgt_dict)
        return decoder
...
class Wav2VecEncoder(FairseqEncoder):
    def __init__(self, cfg: Wav2BartChrConfig, tgt_dict=None):
